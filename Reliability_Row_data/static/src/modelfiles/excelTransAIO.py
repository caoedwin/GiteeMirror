...
try:
    ...
    # print(sheetnum)
    if sheetnum == '2':
        df = pd.read_excel(src_file, header=5, sheet_name=int(sheetnum)).iloc[:, 2:]#‘,’前面是行，后面是列，sheet_name指定sheet，可是是int第几个，可以是名称，header从第几行开始读取
        ...
        columnsRename = {
                         'Category': 'Category', 'Test Title': 'TestTitle', 'Sub test title': 'Subtesttitle', 'Test Item': 'TestItem', 'Priority': 'Priority', 'Release Date': 'ReleaseDate', 'Owner ': 'Owner',
                         # 'Version': 'Version',
                         ...
                         'AIO SW Test plan Matrix for Planning': 'Cusumer', 'Unnamed: 18': 'Commercial', 'Unnamed: 19': 'SDV', 'Unnamed: 20': 'SIT', 'Unnamed: 21': 'Coverage',
                         'Feature Support': 'FeatureSupport', 'Base time-support': 'Basetimesupport', 'TE': 'TE', 'Schedule': 'Schedule',
                         ...
                         'Config-Leverage time': 'ConfigLeveragetime', 'Config-Smart Item占总case比例': 'ConfigSmartItemper', 'Config-Smart time': 'ConfigSmarttime',
                         'Comments': 'Comments', 'Project test SKU-Optimize': 'ProjecttestSKUOptimize', 'Attend time-Optimize': 'AttendtimeOptimize',
                         'Planning after Optimize': 'SKU1', 'Unnamed: 38': 'SKU2', 'Unnamed: 39': 'SKU3', 'Unnamed: 40': 'SKU4', 'Unnamed: 41': 'SKU5',
                         'Unnamed: 42': 'SKU6', 'Unnamed: 43': 'SKU7', 'Unnamed: 44': 'SKU8', 'Unnamed: 45': 'SKU9', 'Unnamed: 46': 'SKU10', 'Config-Retest Cycle': 'ConfigRetestCycle',
        ...
        df.to_excel('uploadAIO.xlsx', sheet_name="sheet1", index=False, engine='xlsxwriter')# engine默认是openpyxl， openpyxl生成的文件DDIS上传不了
    ...
    if sheetnum == '3':
        ...
        df['ReleaseDate'] = df['ReleaseDate'].apply(lambda x: x.strftime('%Y-%m-%d') if 'datetime.datetime' in str(type(x)) else x) # 批量类型转换
