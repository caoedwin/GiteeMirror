...
class ProgressThread(QThread):
    signal = pyqtSignal()
    def befroe(self, sleeptime):#ed
        self.sleeptime = sleeptime#ed
    def run(self):
        print(self.sleeptime,'Run_sleeptime')
        for p in range(100):
            self.signal.emit()
            self.msleep(self.sleeptime)#ed
...
class Progress(QDialog):
    percent = 0
    def __init__(self, text="", sleeptime=100, parent=None):#ed
        ...
        self.ProgressThread.befroe(sleeptime)#ed
...
#     r = Progress(text="<a link='b'>运行中...</a>", sleeptime=10)
#     r.show()
#     sys.exit(app.exec_())
