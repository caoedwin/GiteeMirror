...
class ABOQIL_F(forms.Form):
    ...
    #     ('Others', 'Others'),
    # )
    Product = forms.CharField(label="Product", max_length=100, required=True,
                              widget=forms.TextInput(attrs={'class': 'form-control-new'}))
    Customer = forms.CharField(max_length=20,
                               widget=forms.Select(choices=Customer_list, attrs={'class': 'form-control-new'}))
    ABOQIL_No = forms.CharField(label="ABOQIL_No", max_length=100, required=True,
                                widget=forms.TextInput(attrs={'class': 'form-control-new'}))
    Issue_Description = forms.CharField(label="Issue_Description", required=True,
                                        widget=forms.Textarea(attrs={'class': 'form-control-new'}))
    Root_Cause = forms.CharField(label="Root_Cause", required=True,
                                 widget=forms.Textarea(attrs={'class': 'form-control-new'}))
    Status = forms.CharField(max_length=20,
                             widget=forms.Select(choices=Status_list, attrs={'class': 'form-control-new'}))
