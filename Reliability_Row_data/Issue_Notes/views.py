...
from .forms import Issue_Notes_F
from app01.models import ProjectinfoinDCT
from app01.models import UserInfo
@csrf_exempt
def Issue_Notes_upload(request):
    ...
    weizhi="Issue_Notes/Issue_Notes_upload"
    ...
    Issue_Notes = Issue_Notes_F(request.POST)
    # print(request.method)
    if request.method == 'POST':
        # print(request.POST)
        if 'Upload' in request.POST.keys():
            if Issue_Notes.is_valid():
                Customer = request.POST.get('Customer')
                Project_Code = request.POST.get('Project_Code')
                Platform = request.POST.get('Platform')
                TDMS_NO = request.POST.get('TDMS_NO')
                ...
                Check_dic = {"Customer": Customer, "Project_Code": Project_Code, "Platform": Platform, "TDMS_NO": TDMS_NO,
                                "Issue_Title": Issue_Title
                             }
                Create_dic = {"Customer": Customer, "Project_Code": Project_Code, "Platform": Platform,
                             "TDMS_NO": TDMS_NO, "Issue_Title": Issue_Title,
                              "Root_Cause": Root_Cause, "Solution": Solution, "PL": PL,
                              "Editor": Editor, "Edittime": Edittime
                             }
                if Issue_Notes_M.objects.filter(**Check_dic).first():
                    ...
                    Issue_Notes_M.objects.create(**Create_dic)
                    result = 0
            else:
                cleandata=Issue_Notes.errors

    return render(request, 'Issue_Notes/Issue_Notes_upload.html', locals())

@csrf_exempt
def Issue_Notes_search(request):
    if not request.session.get('is_login', None):
        return redirect('/login/')
    Skin = request.COOKIES.get('Skin_raw')
    # print(Skin)
    if not Skin:
        Skin = "/static/src/blue.jpg"
    weizhi="Issue_Notes/Issue_Notes_search"
    mock_data = [
    ...
    for i in roles:
        ...
        #     canExport = 1
    for i in Issue_Notes_M.objects.all().values("Customer").distinct().order_by("Customer"):
    ...
    if request.method == 'POST':
        if request.POST.get("isGetData") == "SEARCHALERT":
            ...
            Prolist = []
            if Customer:
                for i in Issue_Notes_M.objects.filter(Customer=Customer).values("Project_Code").distinct().order_by(
                        "Project_Code"):
                    Prolist.append(i["Project_Code"])
            else:
                for i in Issue_Notes_M.objects.all().values("Project_Code").distinct().order_by("Project_Code"):
                    Prolist.append(i["Project_Code"])
            # print(Prolist)
            for i in Prolist:
        ...
        if request.POST.get("isGetData") == "SEARCH":
            ...
            for i in Issue_Notes_M.objects.filter(Customer=Customer, Project_Code=Project_Code):
                mock_data.append({
                    "id": i.id, "Customer": i.Customer, "Project_Code": i.Project_Code, "Platform": i.Platform, "TDMS_NO": i.TDMS_NO,
                     "Issue_Title": i.Issue_Title, "Root_Cause": i.Root_Cause, "Solution": i.Solution, "PL": i.PL
                })
    ...
    return render(request, 'Issue_Notes/Issue_Notes_search.html', locals())

@csrf_exempt
def Issue_Notes_edit(request):
    if not request.session.get('is_login', None):
        return redirect('/login/')
    Skin = request.COOKIES.get('Skin_raw')
    # print(Skin)
    if not Skin:
        Skin = "/static/src/blue.jpg"
    weizhi="Issue_Notes/Issue_Notes_edit"
    mock_data = [
        # {"id": "1", "Customer": "C38A", "Project_Code": "TBD", "Platform": "FLY00", "TDMS_NO": "FLY00",
        #  "Issue_Title": "2020", "Root_Cause": "24", "Solution": "1", "PL": "aaaa"},
        # {"id": "2", "Customer": "C38N", "Project_Code": "TBD", "Platform": "FLY00", "TDMS_NO": "FLY00",
    ...
    if request.method == 'POST':
        ...
        if 'SAVE' in str(request.body):
            ...
            update_dic = {
                ...
                "Platform": resdatas["rows"]["Platform"], "TDMS_NO": resdatas["rows"]["TDMS_NO"],
                "Issue_Title": resdatas["rows"]["Issue_Title"], "Root_Cause": resdatas["rows"]["Root_Cause"],
                "Solution": resdatas["rows"]["Solution"], "PL": resdatas["rows"]["PL"],
                "Editor": request.session.get('user_name'), "Edittime": datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            }
            Issue_Notes_M.objects.filter(id=id).update(**update_dic)
            for i in Issue_Notes_M.objects.filter(Customer=Customer, Project_Code=Project_Code):
                mock_data.append({
                    "id": i.id, "Customer": i.Customer, "Project_Code": i.Project_Code, "Platform": i.Platform, "TDMS_NO": i.TDMS_NO,
                     "Issue_Title": i.Issue_Title, "Root_Cause": i.Root_Cause, "Solution": i.Solution, "PL": i.PL
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'Issue_Notes/Issue_Notes_edit.html', locals())