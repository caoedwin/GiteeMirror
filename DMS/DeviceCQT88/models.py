...
class PICS(models.Model):
    ...
    pic = models.ImageField(upload_to='DeviceCQT88/PIC/',verbose_name='图片地址')
...
class DeclarationNofile(models.Model):
    # id = models.AutoField(max_length=10, primary_key=True, verbose_name='id')
    files = models.ImageField(upload_to='DeviceCQT88/DeclarationNo/',verbose_name='文件内容')
...
class DeviceCQT88(models.Model):
    Customer_list = (
        ('', ''),
        ('ABO', 'ABO'),
    )
    ...
    class Meta:
        verbose_name = 'Device_ABO'#不写verbose_name, admin中默认的注册名会加s
        verbose_name_plural = verbose_name
    def __str__(self):
        return '{NID}>>{DevID}'.format(NID=self.NID, DevID=self.DevID)

class DeviceCQT88His(models.Model):
    Result_choice = (
        # ('Select Customer', 'Select Customer'),
        # ('', ''),
        ('Pass', 'Pass'),
