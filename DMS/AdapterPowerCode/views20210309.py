...
def mailattach(subject,from_email, to_email, message, attachname):
    # subject 主题 content 内容 to_addr 是一个列表，发送给哪些人sssssssssssssssssssssssss
    ...
    if request.method == "POST":
        ...
        if request.POST.get('isGetData') == 'BORROW':
            ...
            updatedic = {'Project_Code': request.POST.get('Project'), 'Phase': request.POST.get('Phase'),
                         'BR_Status': '預定確認', 'BR_per': request.session.get('CNname'),
...
def R_Borrowed(request):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCodeBR.objects.filter(BR_per=request.session.get('CNname'), BR_Status='已借出')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
                ...
                guihuanqueren = mock_datalist.filter(BR_Status="歸還確認").count()  # 歸還確認中
            if request.POST.get('isGetData') == 'SEARCH':
                checkAdaPow = {'BR_per': request.session.get('CNname'), 'BR_Status': '已借出'}
                Changjiasearch = request.POST.get('Changjia')
                if Changjiasearch and Changjiasearch != "All":
                    checkAdaPow['Changjia'] = Changjiasearch
                Customerserach = request.POST.get('Customer')
            ...
            if request.POST.get('isGetData') == 'RENEW':
                ...
                # print(BorrowedID.split(','))
                updatedic = {'Project_Code': request.POST.get('Project'), 'Phase': request.POST.get('Phase'),
                             'BR_Status': '續借確認', 'BR_per': request.session.get('CNname'),
...
def R_Return(request):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCodeBR.objects.filter(BR_per=request.session.get('CNname'), BR_Status='歸還確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
                ...
                guihuanqueren = mock_datalist.filter(BR_Status="歸還確認").count()  # 歸還確認中
            if request.POST.get('isGetData') == 'SEARCH':
                checkAdaPow = {'BR_per': request.session.get('CNname'), 'BR_Status': '歸還確認'}
                Changjiasearch = request.POST.get('Changjia')
                if Changjiasearch and Changjiasearch != "All":
                    checkAdaPow['Changjia'] = Changjiasearch
                Customerserach = request.POST.get('Customer')
...
def R_Keep(request):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCodeBR.objects.filter(BR_per=request.session.get('CNname'), BR_Status='續借確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
                ...
                guihuanqueren = mock_datalist.filter(BR_Status="歸還確認").count()  # 歸還確認中
            if request.POST.get('isGetData') == 'SEARCH':
                checkAdaPow = {'BR_per': request.session.get('CNname'), 'BR_Status': '續借確認'}
                Changjiasearch = request.POST.get('Changjia')
                if Changjiasearch and Changjiasearch != "All":
                    checkAdaPow['Changjia'] = Changjiasearch
                Customerserach = request.POST.get('Customer')
...
def R_Destine(request):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCodeBR.objects.filter(BR_per=request.session.get('CNname'), BR_Status='預定確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
                ...
                guihuanqueren = mock_datalist.filter(BR_Status="歸還確認").count()  # 歸還確認中
            if request.POST.get('isGetData') == 'SEARCH':
                checkAdaPow = {'BR_per': request.session.get('CNname'), 'BR_Status': '預定確認'}
                Changjiasearch = request.POST.get('Changjia')
                if Changjiasearch and Changjiasearch != "All":
                    checkAdaPow['Changjia'] = Changjiasearch
                Customerserach = request.POST.get('Customer')
...
def M_Borrow(request):
    ...
    for i in AdapterPowerCodeBR.objects.filter(BR_Status='預定確認').values('BR_per').distinct().order_by('BR_per'):
        selectItem.append({"value": i['BR_per']})
    mock_data = [
        # {"id": "1", "Changjia": "123", "MaterialPN": "", "Description": "", "Power": "", "Number": "", "Location": "",
        #  "Customer": "", "Project_Code": "", "Phase": "", "OAP": "", "Device_Status": "", "BR_Status": "已借用",
        #  "BR_per": "",
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCodeBR.objects.filter(BR_Status='預定確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
...
def M_Return(request):
    ...
    for i in AdapterPowerCodeBR.objects.filter(BR_Status='歸還確認').values('BR_per').distinct().order_by('BR_per'):
        selectItem.append({"value": i['BR_per']})
    mock_data = [
        # {"id": "1", "Changjia": "123", "MaterialPN": "", "Description": "", "Power": "", "Number": "", "Location": "",
        #  "Customer": "", "Project_Code": "", "Phase": "", "OAP": "", "Device_Status": "", "BR_Status": "已借用",
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCodeBR.objects.filter(BR_Status='歸還確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
...
def M_Keep(request):
    ...
    for i in AdapterPowerCodeBR.objects.filter(BR_Status='續借確認').values('BR_per').distinct().order_by('BR_per'):
        selectItem.append({"value": i['BR_per']})
    mock_data = [
        # {"id": "1", "Changjia": "123", "MaterialPN": "", "Description": "", "Power": "", "Number": "", "Location": "",
        #  "Customer": "", "Project_Code": "", "Phase": "", "OAP": "", "Device_Status": "", "BR_Status": "已借用",
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCodeBR.objects.filter(BR_Status='續借確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
