...
@receiver(pre_delete, sender=A32Imgs)
def mymodel_delete(sender, instance, **kwargs):
    # Pass false so FileField doesn't save the model.
    instance.img.delete(False)
class A32files(models.Model):
    files = models.FileField(upload_to="videos/A32/Lesson/", null=True, blank=True, verbose_name="视频内容")
    single = models.CharField(max_length=200, null=True, blank=True, verbose_name='视频名称')
    def __unicode__(self):  # __str__ on Python 3
        return (self.id,self.files)

class A32lesson_learn(models.Model):
    choosecat = (
        ("", ""),
    ...
    Photo = models.ManyToManyField(A32Imgs, related_name='imgs', blank=True, verbose_name='图片表')
    video = models.ManyToManyField(A32files, related_name='video', blank=True, verbose_name='视频表')
    editor = models.CharField(max_length=100)
    edit_time = models.CharField('edit_time', max_length=26, blank=True)
		#upload_to参数为指定的文件服务器上保存路径，如果没有该目录django会自动创建
    class Meta:
        verbose_name = 'A32lesson_learn'#不写verbose_name, admin中默认的注册名会加s
...
class A32TestProjectLL(models.Model):
    Customer_choice = (
    ...
    class Meta:
        verbose_name = 'A32TestProjectLL'  # 不写verbose_name, admin中默认的注册名会加s
        verbose_name_plural = verbose_name
...
class A32lessonlearn_Project(models.Model):
    result_choice = (
        # ('Select Phase', 'Select Phase'),
        ('', ''),
        ('Pass', 'Pass'),
    ...
    Projectinfo = models.ForeignKey("A32TestProjectLL", on_delete=True)
    lesson = models.ForeignKey("A32lesson_learn", on_delete=True)
