...
def TestPlanSW_Edit(request):
    ...
    if request.method == "POST":
        ...
        if request.POST.get('action')=='edit':
            ...
            if 'conStime' in request.POST.keys():
                if request.POST.get('conStime'):
                    updatedate['ConfigSmartTime'] = float(request.POST.get('conStime'))
                else:
            ...
            if 'CRT' in request.POST.keys():
                if request.POST.get('CRT'):
                    updatedate['ConfigRetestSKU'] = float(request.POST.get('CRT'))
                else:
                    updatedate['ConfigRetestSKU'] =None
                ...
                for i in xlsxlist:
                    ...
                    if i['Category2']== "Others (Project Leader update)(1. Base on Golden BIOS/Image's change list)(2. Retest for w/o solution’s issue ) for full function test":
                        ...
                        if 'ItemNo_d' in i.keys() and 'Item_d' in i.keys() and 'TestItems' in i.keys():
                            ...
                            # edit
                            if editplan:
                                # print('edit')/TestPlanSW/TestPlanSW-edit/
...
def TestPlanSW_search(request):
    ...
    if request.method == "GET":
        ...
        #     return HttpResponse(json.dumps(updateData), content_type="application/json")
            ...
            basetimesum = TestItemSW.objects.filter(Customer=Customer,Phase=Phase).values('Category2').annotate(
                Sum('BaseTime')).order_by('Category2')
            BTSsum = TestPlanSW.objects.filter(Projectinfo=ProjectSum).values('Items__Category2').annotate(
                Sum('BaseTimeSupport')).order_by('Items__Category2')
            TFCsum = TestPlanSW.objects.filter(Projectinfo=ProjectSum).values('Items__Category2').annotate(
                Sum('TimewConfigFollowmatrix')).order_by('Items__Category2')
            CATsum = TestPlanSW.objects.filter(Projectinfo=ProjectSum).values('Items__Category2').annotate(
                Sum('ConfigAutomationTime')).order_by('Items__Category2')
            CLTsum = TestPlanSW.objects.filter(Projectinfo=ProjectSum).values('Items__Category2').annotate(
                Sum('ConfigLeverageTime')).order_by('Items__Category2')
            CSTsum = TestPlanSW.objects.filter(Projectinfo=ProjectSum).values('Items__Category2').annotate(
                Sum('ConfigSmartTime')).order_by('Items__Category2')
            ATOsum = TestPlanSW.objects.filter(Projectinfo=ProjectSum).values('Items__Category2').annotate(
                Sum('AttendTimeOptimize')).order_by('Items__Category2')
            CRTsum = TestPlanSW.objects.filter(Projectinfo=ProjectSum).values('Items__Category2').annotate(
                Sum('ConfigRetestTime')).order_by('Items__Category2')
            # print(basetimesum)
            # print(basetimesum[1])
            # print(BTSsum)
            ...
            for i in basetimesum:
                ...
                if not BTSsum[Num]['BaseTimeSupport__sum']:
                    ...
                    BTS1 = BTSsum[Num]['BaseTimeSupport__sum']
                if not TFCsum[Num]['TimewConfigFollowmatrix__sum']:
                    TFC1 = 0.00
                else:
                    TFC1 = TFCsum[Num]['TimewConfigFollowmatrix__sum']
                if not CATsum[Num]['ConfigAutomationTime__sum']:
                    CAT1 = 0.00
                else:
                    CAT1 = CATsum[Num]['ConfigAutomationTime__sum']
                if not CLTsum[Num]['ConfigLeverageTime__sum']:
                    CLT1 = 0.00
                else:
                    CLT1 = CLTsum[Num]['ConfigLeverageTime__sum']
                if not CSTsum[Num]['ConfigSmartTime__sum']:
                    CST1 = 0.00
                else:
                    CST1 = CSTsum[Num]['ConfigSmartTime__sum']
                if not ATOsum[Num]['AttendTimeOptimize__sum']:
                    ATO1 = 0.00
                else:
                    ATO1 = ATOsum[Num]['AttendTimeOptimize__sum']
                if not CRTsum[Num]['ConfigRetestTime__sum']:
                    CRT1 = 0.00
                else:
                    CRT1 = CRTsum[Num]['ConfigRetestTime__sum']
                if not FFD:
                    HC = 0
                else:
                    HC = round(ATO1 / 6 / TestProjectSW.objects.filter(
                        **dic_ProjectSum).first().Full_Function_Duration, 2)
                allSum.append({'category': i['Category2'], 'basetime': i['BaseTime__sum'], 'BTS': BTS1,
                               'TFC': TFC1, 'CAT': CAT1,
                               'CLT': CLT1, 'CST': CST1,
                               'ATO': ATO1, 'CRT': CRT1,
            ...
            if Phase == 'FFRT':
                if Retestitems:
                    ...
                    if not FFD:
                        ...
                        if not RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('AttendTimeOptimize'))['AttendTimeOptimize__sum']:
                            HCR = round(RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('AttendTimeOptimize'))['AttendTimeOptimize__sum'] / 6 / TestProjectSW.objects.filter(
                                **dic_ProjectSum).first().Full_Function_Duration, 2)
                        else:
                            HCR = 0
                    allSum.append({
                        ...
                        'basetime': RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('BaseTime'))['BaseTime__sum'], 'BTS': RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('BaseTimeSupport'))['BaseTimeSupport__sum'],
                        'TFC': RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('TimewConfigFollowmatrix'))['TimewConfigFollowmatrix__sum'], 'CAT': RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('ConfigAutomationTime'))['ConfigAutomationTime__sum'],
                        'CLT': RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('ConfigLeverageTime'))['ConfigLeverageTime__sum'], 'CST': RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('ConfigSmartTime'))['ConfigSmartTime__sum'],
                        'ATO': RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('AttendTimeOptimize'))['AttendTimeOptimize__sum'], 'CRT': RetestItemSW.objects.filter(**dicSum_check).aggregate(Sum('ConfigRetestTime'))['ConfigRetestTime__sum'],
            ...
            if request.GET.get("category") == "Others (Project Leader update)(1. Base on Golden BIOS/Image's change list)(2. Retest for w/o solution’s issue ) for full function test":
                ...
                if Projectinfos:
                    ...
                    Sums['basetimeSum'] = Iteminfos.aggregate(Sum('BaseTime'))[
                        'BaseTime__sum']
                    Sums['unattendtimeSum'] = Iteminfos.aggregate(Sum('TDMSUnattendedTime'))[
                        'TDMSUnattendedTime__sum']
                    Sums['basetimeASum'] = Iteminfos.aggregate(Sum('BaseAotomationTime1SKU'))[
