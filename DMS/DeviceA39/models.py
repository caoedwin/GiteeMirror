...
class PICS(models.Model):
    ...
    pic = models.ImageField(upload_to='DeviceA39/PIC/',verbose_name='图片地址')
...
class DeclarationNofile(models.Model):
    # id = models.AutoField(max_length=10, primary_key=True, verbose_name='id')
    files = models.ImageField(upload_to='DeviceA39/DeclarationNo/',verbose_name='文件内容')
...
class DeviceA39(models.Model):
    ...
    LSTA = models.CharField(max_length=256, choices=LSTA_choice, null=True, blank=True, verbose_name='A39 SW Test lab device Audit list: Require state(Must, Optional)')
    ...
    class Meta:
        verbose_name = 'Device_A39'#不写verbose_name, admin中默认的注册名会加s
        verbose_name_plural = verbose_name
    def __str__(self):
        return '{NID}>>{DevID}'.format(NID=self.NID, DevID=self.DevID)

class DeviceA39His(models.Model):
    Result_choice = (
        # ('Select Customer', 'Select Customer'),
        # ('', ''),
        ('Pass', 'Pass'),
