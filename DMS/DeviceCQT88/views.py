...
from .models import DeviceIntfCtgryList, DeviceDevCtgryList, DeviceDevpropertiesList, DeviceDevVendorList, \
    DeviceDevsizeList
...
from .models import DeviceCQT88, DeviceCQT88His, PICS, DeviceIntfCtgryList, DeviceDevCtgryList, DeviceDevpropertiesList, \
    DeviceDevVendorList, DeviceDevsizeList
...
def BorrowedDeviceCQT88(request):
    ...
    if DeviceCQT88.objects.all():
        for i in DeviceCQT88.objects.values("BrwStatus").distinct().order_by("BrwStatus"):
        ...
        if 'first' in str(request.body):
            ...
            if checkAdaPow:
                ...
                if "IntfCtgry" in checkAdaPow.keys() and "DevCtgry" in checkAdaPow.keys() and "Devproperties" in checkAdaPow.keys() and "DevVendor" in checkAdaPow.keys() and "Devsize" in checkAdaPow.keys():
                    mock_datalist = DeviceCQT88.objects.filter(
                    ...
                    mock_datalist = DeviceCQT88.objects.filter(**checkAdaPow).filter(DevStatus__in=["Good", "Fixed", 'Long'])
            else:
                mock_datalist = DeviceCQT88.objects.all().filter(DevStatus__in=["Good", "Fixed", 'Long'])
            # print(mock_datalist)
            ...
            for i in BorrowedID.split(','):
                ...
                try:
                    with transaction.atomic():
                        # print(updatedic)
                        DeviceCQT88.objects.filter(id=i).update(**updatedic)
                        alert = 0
                except:
                    # print('2')
    ...
    return render(request, 'DeviceCQT88/BorrowedDeviceCQT88.html', locals())
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                ...
                # else:
                #     mock_datalist = DeviceCQT88.objects.all()
                # print(mock_datalist)
                for i in mock_datalist:
                    # Photolist = []
                    # for h in i.Photo.all():
            ...
            if request.POST.get('isGetData') == 'SEARCH5':
                ...
                # mock_data
                if IntfCtgry and IntfCtgry != "All" and Devproperties and Devproperties != "All":
                    mock_datalist = DeviceCQT88.objects.filter(**checkAdaPowfirst).filter(
                    ...
                    mock_datalist = DeviceCQT88.objects.filter(**checkAdaPowfirst)
                if checkAdaPow:
                    # print(checkAdaPow)
                    # mock_datalist = DeviceCQT88.objects.filter(**checkAdaPow)
                    mock_datalist = mock_datalist.filter(**checkAdaPow)
            ...
            if request.POST.get('isGetData') == 'RENEW':
                ...
                for i in RenewId.split(','):
                    updatedic['Last_BR_per'] = DeviceCQT88.objects.filter(id=i).first().Usrname
                    updatedic['Last_BR_per_code'] = DeviceCQT88.objects.filter(id=i).first().BR_per_code
                    updatedic['Last_Predict_return'] = DeviceCQT88.objects.filter(id=i).first().Plandate
                    updatedic['Last_Borrow_date'] = DeviceCQT88.objects.filter(id=i).first().Btime
                    updatedic['Last_Return_date'] = datetime.datetime.now().date()
                    updatedic['Last_ProjectCode'] = DeviceCQT88.objects.filter(id=i).first().ProjectCode
                    updatedic['Last_Phase'] = DeviceCQT88.objects.filter(id=i).first().Phase
                    try:
                        with transaction.atomic():
                            DeviceCQT88.objects.filter(id=i).update(**updatedic)
                            alert = 0
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceCQT88/R_Borrowed.html', locals())
...
def R_Return(request):
    ...
    if request.method == "POST":
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceCQT88/R_Return.html', locals())
...
def R_Keep(request):
    ...
    if request.method == "POST":
            ...
            try:
                ...
                if 'CancelRenew' in str(request.body):
                    ...
                    for i in responseData['Renew']:
                        updatedic['Usrname'] = DeviceCQT88.objects.filter(id=i).first().Last_BR_per
                        updatedic['BR_per_code'] = DeviceCQT88.objects.filter(id=i).first().Last_BR_per_code
                        updatedic['ProjectCode'] = DeviceCQT88.objects.filter(id=i).first().Last_ProjectCode
                        updatedic['Phase'] = DeviceCQT88.objects.filter(id=i).first().Last_Phase
                        updatedic['Plandate'] = DeviceCQT88.objects.filter(id=i).first().Last_Predict_return
                        updatedic['Btime'] = DeviceCQT88.objects.filter(id=i).first().Last_Borrow_date
                        updatedic['Rtime'] = DeviceCQT88.objects.filter(id=i).first().Last_Return_date
                        try:
                            with transaction.atomic():
                                # print(updatedic)
                                DeviceCQT88.objects.filter(id=i).update(**updatedic)
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceCQT88/R_Keep.html', locals())
...
def R_Destine(request):
    ...
    if request.method == "POST":
            ...
            try:
                ...
                if 'CancelBook' in str(request.body):
                    ...
                    for i in responseData['BookId']:
                        ...
                        updatedic['ProjectCode'] = DeviceCQT88.objects.filter(id=i).first().Last_ProjectCode
                        updatedic['Phase'] = DeviceCQT88.objects.filter(id=i).first().Last_Phase
                        # updatedic['Predict_return'] = DeviceCQT88.objects.filter(id=i).first().Last_Predict_return
                        updatedic['Btime'] = DeviceCQT88.objects.filter(id=i).first().Last_Borrow_date
                        updatedic['Rtime'] = DeviceCQT88.objects.filter(id=i).first().Last_Return_date
                        # print(updatedic)
                        try:
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceCQT88/R_Destine.html', locals())
...
def M_Borrow(request):
    ...
    for i in DeviceCQT88.objects.filter(BrwStatus='預定確認中').values('Usrname', 'BR_per_code').distinct().order_by(
            'BR_per_code'):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = DeviceCQT88.objects.filter(BrwStatus='預定確認中')
                for i in mock_datalist:
                    # Photolist = []
                    # for h in i.Photo.all():
                    #     Photolist.append(
            ...
            try:
                ...
                if 'ChangeDelivery' in str(request.body):
                    ...
                    for i in responseData['params']:
                        ...
                        # updatedic['Last_Return_date'] = DeviceCQT88.objects.filter(id=i).first().Return_date
                        try:
                            with transaction.atomic():
                                # print(updatedic)
                                DeviceCQT88.objects.filter(id=i).update(**updatedic)
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceCQT88/M_Borrow.html', locals())
...
def M_Return(request):
    ...
    for i in DeviceCQT88.objects.filter(BrwStatus='歸還確認中').values('Usrname', 'BR_per_code').distinct().order_by(
            'BR_per_code'):
        selectItem.append({"value": i['Usrname'], 'number': i['BR_per_code']})
    mock_data = [
    ...
    if request.method == "POST":
        if request.POST:
            ...
            try:
                ...
                if 'ChangeStorages' in str(request.body):
                    ...
                    for i in responseData['params']:
                        # updatedic['Last_BR_per'] = DeviceCQT88.objects.filter(id=i).first().BR_per
                        # updatedic['Last_Predict_return'] = DeviceCQT88.objects.filter(id=i).first().Predict_return
                        # updatedic['Last_Borrow_date'] = datetime.datetime.now().date(),
                        updatedic['Last_Return_date'] = datetime.datetime.now().date()

                        if DeviceCQT88.objects.filter(id=i).first().uscyc:
                            uscyc = int(DeviceCQT88.objects.filter(id=i).first().uscyc)
                        else:
                            uscyc = 0
                        if DeviceCQT88.objects.filter(id=i).first().UsrTimes:
                            UsrTimes = int(DeviceCQT88.objects.filter(id=i).first().UsrTimes)
                        else:
                            UsrTimes = 0
                        updatedic['uscyc'] = str(uscyc + int(DeviceCQT88.objects.filter(id=i).first().Last_uscyc))
                        updatedic['UsrTimes'] = str(UsrTimes + 1)

                        Devicebyid = DeviceCQT88.objects.filter(id=i).first()
                        updatedic_His = {
                            "NID": Devicebyid.NID, "DevID": Devicebyid.DevID,
                            "DevModel": Devicebyid.DevModel, "DevName": Devicebyid.DevName,
                            "uscyc": DeviceCQT88.objects.filter(id=i).first().Last_uscyc,
                        ...
                        try:
                            with transaction.atomic():
                                if DeviceCQT88His.objects.filter(**updatedic_His_check):
                                    pass  # 防止历史记录存成功了，Device更新失败，需要重复操作。
                                else:
                                    DeviceCQT88His.objects.create(**updatedic_His)
                                DeviceCQT88.objects.filter(id=i).update(**updatedic)
                                alert = 0
                        except:
                            alert = '此数据%s正被其他使用者编辑中...' % i
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceCQT88/M_Return.html', locals())
...
def M_Keep(request):
    ...
    for i in DeviceCQT88.objects.filter(BrwStatus='續借確認中').values('Usrname', 'BR_per_code').distinct().order_by(
            'BR_per_code'):
        selectItem.append({"value": i['Usrname'], 'number': i['BR_per_code']})
    mock_data = [
        # {"id": "2", "Customer": "C38", "Plant": "KS",
    ...
    if request.method == "POST":
                ...
                if 'EnsureRenew' in str(request.body):
                    ...
                    for i in responseData['params']:
                        ...
                        updatedic_His = {
                            "NID": Devicebyid.NID, "DevID": Devicebyid.DevID,
                            "DevModel": Devicebyid.DevModel, "DevName": Devicebyid.DevName,
                            # "uscyc": DeviceCQT88.objects.filter(id=i).first().uscyc,
                            "Btime": Devicebyid.Last_Borrow_date, "Plandate": Devicebyid.Last_Predict_return,
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceCQT88/M_Keep.html', locals())
...
def M_Category(request):
    ...
    if request.method == "POST":
        ...
        if request.body:
            ...
            if 'addSubmit2' in str(request.body):
                ...
                if DeviceDevCtgryList.objects.filter(DevCtgry=DevCtgry, IntfCtgry_P=IntfCtgry_P):
                    ...
                    DeviceDevCtgryList.objects.create(DevCtgry=DevCtgry, IntfCtgry_P=DeviceIntfCtgryList.objects.filter(
                        IntfCtgry=IntfCtgry).first())
            ...
            if 'editSubmit3' in str(request.body):
                ...
                if DeviceDevpropertiesList.objects.filter(**NewDevpropertiesUpdatedic):
                    errormessae = """IntfCtgry:%s>DevCtgry:%s>Devproperties:%s 已经存在 """ % (
                    NewIntfCtgry, NewDevCtgry, NewDevproperties)
                    print(errormessae)
                else:
            ...
            if 'addSubmit3' in str(request.body):
                ...
                if DeviceDevpropertiesList.objects.filter(Devproperties=Devproperties, DevCtgry_P=DevCtgry_P):
                    errormessae = """IntfCtgry:%s>DevCtgry:%s>Devproperties:%s 已经存在 """ % (
                    IntfCtgry, DevCtgry, Devproperties)
                    print(errormessae)
                else:
                    # print(DeviceIntfCtgryList.objects.filter(IntfCtgry=IntfCtgry).first(),type(DeviceIntfCtgryList.objects.filter(IntfCtgry=IntfCtgry).first()))
                    DeviceDevpropertiesList.objects.create(Devproperties=Devproperties,
                                                           DevCtgry_P=DeviceDevCtgryList.objects.filter(
                                                               DevCtgry=DevCtgry, IntfCtgry_P=IntfCtgry_P).first())
            ...
            if 'editSubmit4' in str(request.body):
                ...
                IntfCtgry_P = DeviceIntfCtgryList.objects.filter(IntfCtgry=NewIntfCtgry).first()
                DevCtgry_P = DeviceDevCtgryList.objects.filter(DevCtgry=NewDevCtgry, IntfCtgry_P=IntfCtgry_P).first()
                Devproperties_P = DeviceDevpropertiesList.objects.filter(Devproperties=NewDevproperties,
                                                                         DevCtgry_P=DevCtgry_P).first()
                ...
                if DeviceDevVendorList.objects.filter(**NewDevVendorUpdatedic):
                    errormessae = """IntfCtgry:%s>DevCtgry:%s>Devproperties:%s>DevVendor:%s 已经存在 """ % (
                    NewIntfCtgry, NewDevCtgry, NewDevproperties, NewDevVendor)
                    print(errormessae)
                else:
            ...
            if 'addSubmit4' in str(request.body):
                ...
                if DeviceDevVendorList.objects.filter(DevVendor=DevVendor, Devproperties_P=Devproperties_P):
                    errormessae = """IntfCtgry:%s>DevCtgry:%s>Devproperties:%s>DevVendor:%s 已经存在 """ % (
                    IntfCtgry, DevCtgry, Devproperties, DevVendor)
                    print(errormessae)
                else:
                    # print(DeviceIntfCtgryList.objects.filter(IntfCtgry=IntfCtgry).first(),type(DeviceIntfCtgryList.objects.filter(IntfCtgry=IntfCtgry).first()))
            ...
            if 'editSubmit5' in str(request.body):
                ...
                Devproperties_P = DeviceDevpropertiesList.objects.filter(Devproperties=NewDevproperties,
                                                                         DevCtgry_P=DevCtgry_P).first()
                DevVendor_P = DeviceDevVendorList.objects.filter(DevVendor=NewDevVendor,
                ...
                if DeviceDevsizeList.objects.filter(**NewDevsizeUpdatedic):
                    errormessae = """IntfCtgry:%s>DevCtgry:%s>Devproperties:%s>DevVendor:%s>DevVendor:%s 已经存在 """ % (
                    NewIntfCtgry, NewDevCtgry, NewDevproperties, NewDevVendor, NewDevsize)
                    print(errormessae)
                else:
            ...
            if 'addSubmit5' in str(request.body):
                ...
                if DeviceDevsizeList.objects.filter(Devsize=Devsize, DevVendor_P=DevVendor_P):
                    errormessae = """IntfCtgry:%s>DevCtgry:%s>Devproperties:%s>DevVendor:%s>Devsize:%s 已经存在 """ % (
                        ...
                        for i in uploadxlsxlist:
                            ...
                            if DeviceDevsizeList.objects.filter(Devsize=Devsize, DevVendor_P=DevVendor_P):
                                ...
                                if not Devproperties_P:
                                    ...
                                    Devproperties_P = DeviceDevpropertiesList.objects.filter(
        ...
        # print(data)
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceCQT88/M_Category.html', locals())
...
def M_edit(request):
    ...
    for i in UserInfo.objects.filter(role__name="Device_LNV_Users").values('account', 'CNname').distinct().order_by(
            'CNname'):
    ...
    if DeviceCQT88.objects.all():
        for i in DeviceCQT88.objects.values("DevStatus").distinct().order_by("DevStatus"):
    ...
    errMsg = ''
    errMsgNumber = ''  # 新增弹框
    # print(DeviceCQT88._meta.fields)
    # # print([f.name for f DeviceCQT88._meta.fields])
    # iii=0
    # for i in DeviceCQT88._meta.fields:
    #     print(i.name, type(i), i.get_internal_type())
    # iii+=1
    # print(iii)
    # print(DeviceCQT88._meta.get_fields())
    # print(request.method)
    if request.method == "POST":
        # print(request.POST)
        # print(request.body)
        if request.POST:
            ...
            if request.POST.get('isGetData') == 'SearchJiLian':
                ...
                DevCtgry = request.POST.get('DevCtgry')
                DevCtgrysearch = DeviceDevCtgryList.objects.filter(DevCtgry=DevCtgry,
                                                                   IntfCtgry_P=IntfCtgrysearch).first()
                # print(DevCtgrysearch)
                Devproperties = request.POST.get('Devproperties')
                Devpropertiessearch = DeviceDevpropertiesList.objects.filter(Devproperties=Devproperties,
                                                                             DevCtgry_P=DevCtgrysearch).first()
                # print(Devpropertiessearch)
                for i in DeviceDevVendorList.objects.filter(Devproperties_P=Devpropertiessearch).values(
                        'DevVendor').distinct().order_by('DevVendor'):  # 其实去不去重无所谓，都是唯一的
                    # print(i)
                    sizelist = []
                    if DeviceDevsizeList.objects.filter(DevVendor_P__DevVendor=i['DevVendor']):
                        for j in DeviceDevsizeList.objects.filter(DevVendor_P__DevVendor=i['DevVendor']).values(
                                'Devsize').distinct().order_by('Devsize'):  # 其实去不去重无所谓，都是唯一的
            ...
            if request.POST.get('isGetData') == 'SEARCH':
                ...
                #     # mock_datalist = DeviceCQT88.objects.filter(**checkAdaPow)
                #     if "IntfCtgry" in checkAdaPow.keys() and "DevCtgry" in checkAdaPow.keys() and "Devproperties" in checkAdaPow.keys() and "DevVendor" in checkAdaPow.keys() and "Devsize" in checkAdaPow.keys():
                #         mock_datalist = DeviceCQT88.objects.filter(Q(IntfCtgry__icontains=checkAdaPow['IntfCtgry'])&Q(DevCtgry=checkAdaPow['DevCtgry'])
            ...
            if request.POST.get('action') == "submit":
                ...
                # print(createdic)
                if DeviceCQT88.objects.filter(NID=NID):
                    errMsgNumber = """設備序號已存在"""
                else:
                    # print(createdic)
                    DeviceCQT88.objects.create(**createdic)
                    # print(Photo)
                    # if Photo:
                    ...
                    #         empt.pic = f
                    #         empt.save()
                    #         DeviceCQT88.objects.filter(Number=Number).first().Photo.add(empt)
            ...
            if request.POST.get('action') == "submit1":
                ...
                # if DeviceCQT88.objects.filter(Number=Number):
                #     errMsgNumber = """编号已存在"""
                # else:
                # print(updatedic)
                try:
                    with transaction.atomic():
                        DeviceCQT88.objects.filter(id=id).update(**updatedic)
                        alert = 0
                except:
                    alert = '此数据%s正被其他使用者编辑中...' % id
                # print(Photo)
                # if Photo:
                #     for m in DeviceCQT88.objects.filter(id=id).first().Photo.all():  # 每次接受图片前清除原来的图片，而不是叠加
                #         # print(m.id)
                #         PICS.objects.filter(
                #             id=m.id).delete()
                #     for f in Photo:  # 数据上允许传多张照片，实际生产环境中头像只会传一张
                ...
                #         empt.pic = f
                #         empt.save()
                #         DeviceCQT88.objects.filter(id=id).first().Photo.add(empt)
            ...
            if request.POST.get('isGetData') == "alertData":
                ...
                NID = request.POST.get('NID')
                # print(NID)
                if DeviceCQT88His.objects.filter(NID=NID):
                    for i in DeviceCQT88His.objects.filter(NID=NID):
                        if i.Btime:
                            Btime_str = str(i.Btime)
                        else:
                            Btime_str = ''
            ...
            try:
                ...
                if 'MUTICANCEL' in str(request.body):
                    ...
                    for i in responseData['params']:
                        # print(i)
                        DeviceCQT88.objects.get(id=i).delete()
                ...
                if 'ExcelData' in str(request.body):
                    ...
                    if startupload:
                        for i in uploadxlsxlist:
                            ...
                            if DeviceCQT88.objects.filter(**Check_dic):  # 已经存在覆盖
                                # pass
                                DeviceCQT88.objects.filter(
                                    **Check_dic).update(**i)
                            else:  # 新增
                                DeviceCQT88.objects.create(**i)
                        errMsg = '上傳成功'
        ...
        return HttpResponse(json.dumps(data), content_type="application/json")
    return render(request, 'DeviceCQT88/M_edit.html', locals())
