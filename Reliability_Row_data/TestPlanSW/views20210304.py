    ...
    if request.method == "POST":
            ...
            if Customer == "C38(AIO)" or Customer == "C38(AIO)-T88":
                if not timerange[0]:
                    ...
                    if Customer:
                        ...
                        #         "Project"):
                        #     Prolist.append({"Project": i["Project"], "Phase": i["Phase"]})
                        for i in TestProjectSWAIO.objects.filter(Customer=Customer).filter(Gerber__range=timerange).values(
                        ...
                        #     Prolist.append({"Project": i["Project"], "Phase": i["Phase"]})
                        for i in TestProjectSWAIO.objects.filter(Gerber__range=timerange).values(
                                "Project").distinct().order_by(
                            "Project"):
                            for j in TestProjectSWAIO.objects.filter(Project=i["Project"]):
                                Prolist.append({"Project": i["Project"], "Phase": j.Phase})
        ...
        if 'getMsg' in str(request.body):#前端穿Jason数据
            ...
            if Customer == "C38(AIO)" or Customer == "C38(AIO)-T88":
                ...
                Projectlist_top = list(set(Projectlist_top))
                Phaselist_top = list(set(Phaselist_top))
                Phasesortorder = {'B(SDV)': 0, 'C(SIT)': 1, 'EELP+': 2, }
                # print(Projectlist_top)
                Phaselist_top.sort(key=lambda x: Phasesortorder[x])
                # print(Phaselist_top)
                for i in Projectlist_top:
                ...
                for i in Projectlist:
                    ...
                    if TestProjectSW.objects.filter(**check_dic_Pro).first():
                        check_dic = {"Customer": Customer, "Phase": Phase,
                                     "Projectinfo": TestProjectSW.objects.filter(**check_dic_Pro).first()}
                        check_dic_BS = {"Customer": Customer, "Phase": Phase,
                                     "Projectinfo": TestProjectSW.objects.filter(**check_dic_Pro).first(), "Owner": "DQA"}
                        CodeBS = 0
                        CodeFS = 0
                        CodeTCM = 0
                        CodeCAT = 0
        ...
        #     return HttpResponse(json.dumps(updateData), content_type="application/json")
            ...
            if 'FFRT' in Phase:#FFRT，2，3，4，5，6上面的都一样，只是多了个Others，category,要在RetestItem里面算
                if Retestitems:
                    ...
                    dicSum_check_BS = {'Projectinfo': ProjectSum, "Owner": "DQA"}
            ...
            if TestPlanSW.objects.filter(Projectinfo=ProjectSum, Owner="DQA").aggregate(Sum('BaseTime'))['BaseTime__sum']:
                basetime_Total = round(TestPlanSW.objects.filter(Projectinfo=ProjectSum, Owner="DQA").aggregate(Sum('BaseTime'))['BaseTime__sum']/60, 0)
            else:
                basetime_Total = 0
...
def TestPlanSW_Edit_AIO(request):
    ...
    newContents = [
        ...
        #  "Release_Date": "2018/12/21", "Owner": "DQA", "Attend_Time": 66, "Unattend_Time": 0, "Automation": 0,
        #  "Attend_Time_hrs": 1.1,
...
def TestPlanSW_search_AIO(request):
    ...
    if request.method == "GET":
        ...
        if request.GET.get("action") == "getContent":
            ...
            basetimesum = TestPlanSWAIO.objects.filter(Projectinfo=ProjectSum, Owner="DQA").values('Category').annotate(
            ...
            for i in basetimesum:
                ...
                Num += 1
            basetimesumTotal = TestPlanSWAIO.objects.filter(Projectinfo=ProjectSum, Owner="DQA").values('Category').annotate(
                Sum('AT_AttendTime')).order_by('Category')
            allSum.insert(0, {'category': 'Total',
                              'basetime': round(TestPlanSWAIO.objects.filter(Projectinfo=ProjectSum, Owner="DQA").aggregate(Sum('AT_AttendTime'))['AT_AttendTime__sum']/60, 0),
