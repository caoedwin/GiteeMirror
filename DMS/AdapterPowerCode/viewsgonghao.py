...
from .models import AdapterPowerCode, PICS
...
def BorrowedAdapter(request):
    ...
    for i in AdapterPowerCode.objects.all().values("Customer").distinct().order_by("Customer"):
        customerchajialist = []
        for j in AdapterPowerCode.objects.filter(Customer=i["Customer"]).values("Changjia").distinct().order_by(
                "Changjia"):
            changjiapnlist = []
            for k in AdapterPowerCode.objects.filter(Customer=i["Customer"], Changjia=j["Changjia"]).values(
    ...
    for i in AdapterPowerCode.objects.all().values("Power").distinct().order_by("Power"):
    ...
    if request.method == "POST":
        if request.POST.get('isGetData') == 'first':
            ...
            # mock_data
            mock_datalist = AdapterPowerCode.objects.all()
            ...
            jieyongyuding = mock_datalist.filter(BR_Status="借用預定").count()  # 借用預定中
        ...
        if request.POST.get('isGetData') == 'SEARCH':
            ...
            # mock_data
            if checkAdaPow:
                mock_datalist = AdapterPowerCode.objects.filter(**checkAdaPow)
            else:
                mock_datalist = AdapterPowerCode.objects.all()
            for i in mock_datalist:
                Photolist = []
            ...
            for i in BorrowedID.split(','):
                ...
                try:
                    with transaction.atomic():
                        # print(updatedic)
                        AdapterPowerCode.objects.filter(id=i).update(**updatedic)
                        alert = 0
                except:
                    # print('2')
...
def R_Borrowed(request):
    ...
    for i in AdapterPowerCode.objects.all().values("Customer").distinct().order_by("Customer"):
        customerchajialist = []
        for j in AdapterPowerCode.objects.filter(Customer=i["Customer"]).values("Changjia").distinct().order_by("Changjia"):
            changjiapnlist = []
            for k in AdapterPowerCode.objects.filter(Customer=i["Customer"],Changjia=j["Changjia"]).values("MaterialPN").distinct().order_by("MaterialPN"):
                changjiapnlist.append((k["MaterialPN"]))
            customerchajialist.append({"Changjia": j['Changjia'], "PN": changjiapnlist})
        selectItem[i["Customer"]] = customerchajialist
    for i in AdapterPowerCode.objects.all().values("Power").distinct().order_by("Power"):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCode.objects.filter(BR_per=request.session.get('CNname'), BR_per_code=request.session.get('account'), BR_Status='已借出')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
            ...
            if request.POST.get('isGetData') == 'RENEW':
                ...
                for i in RenewId.split(','):
                    # updatedic['Last_BR_per'] = AdapterPowerCode.objects.filter(id=i).first().BR_per
                    # updatedic['Last_Predict_return'] = AdapterPowerCode.objects.filter(id=i).first().Predict_return
                    # updatedic['Last_Borrow_date'] = AdapterPowerCode.objects.filter(id=i).first().Borrow_date
...
def R_Return(request):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCode.objects.filter(BR_per=request.session.get('CNname'), BR_per_code=request.session.get('account'), BR_Status='歸還確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
...
def R_Keep(request):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCode.objects.filter(BR_per=request.session.get('CNname'), BR_per_code=request.session.get('account'),BR_Status='續借確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
            ...
            try:
                ...
                if 'CancelRenew' in str(request.body):
                    ...
                    for i in responseData['Renew']:
                        updatedic['BR_per'] = AdapterPowerCode.objects.filter(id=i).first().Last_BR_per
                        updatedic['BR_per_code'] = AdapterPowerCode.objects.filter(id=i).first().Last_BR_per_code
                        updatedic['Predict_return'] = AdapterPowerCode.objects.filter(id=i).first().Last_Predict_return
                        updatedic['Borrow_date'] = AdapterPowerCode.objects.filter(id=i).first().Last_Borrow_date
                        updatedic['Return_date'] = AdapterPowerCode.objects.filter(id=i).first().Last_Return_date
                        try:
                            with transaction.atomic():
                                AdapterPowerCode.objects.filter(id=i).update(**updatedic)
                                alert = 0
...
def R_Destine(request):
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCode.objects.filter(BR_per=request.session.get('CNname'), BR_per_code=request.session.get('account'), BR_Status='預定確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
...
def M_Borrow(request):
    ...
    for i in AdapterPowerCode.objects.filter(BR_Status='預定確認').values('BR_per', 'BR_per_code').distinct().order_by('BR_per_code'):
        selectItem.append({"value": i['BR_per'], "codenum": i["BR_per_code"]})
    mock_data = [
        # {"id": "1", "Changjia": "123", "MaterialPN": "", "Description": "", "Power": "", "Number": "", "Location": "",
        #  "Customer": "", "Project_Code": "", "Phase": "", "OAP": "", "Device_Status": "", "BR_Status": "已借用",
        #  "BR_per": "",
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCode.objects.filter(BR_Status='預定確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
            ...
            if request.POST.get('isGetData') == 'SEARCH':
                ...
                if request.POST.get('Borrower'):
                    checkAdaPow['BR_per'] = request.POST.get('Borrower')
                    if request.POST.get('BorrowerNum', None):
                        checkAdaPow['BR_per_per'] = request.POST.get('BorrowerNum')
                # Changjiasearch = request.POST.get('Changjia')
                # if Changjiasearch and Changjiasearch != "All":
                #     checkAdaPow['Changjia'] = Changjiasearch
                # Customerserach = request.POST.get('Customer')
            ...
            try:
                ...
                if 'ChangeDelivery' in str(request.body):
                    ...
                    for i in responseData['params']:
                        # print(i)
                        updatedic['Last_BR_per'] = AdapterPowerCode.objects.filter(id=i).first().BR_per
                        updatedic['Last_BR_per_code'] = AdapterPowerCode.objects.filter(id=i).first().BR_per_code
                        updatedic['Last_Predict_return'] = AdapterPowerCode.objects.filter(id=i).first().Predict_return
                        updatedic['Last_Borrow_date'] = datetime.datetime.now().date()
                        # updatedic['Last_Return_date'] = AdapterPowerCode.objects.filter(id=i).first().Return_date
                        try:
                            with transaction.atomic():
                                # print(updatedic)
                                AdapterPowerCode.objects.filter(id=i).update(**updatedic)
...
def M_Return(request):
    ...
    for i in AdapterPowerCode.objects.filter(BR_Status='歸還確認').values('BR_per', 'BR_per_code').distinct().order_by('BR_per_code'):
        selectItem.append({"value": i['BR_per'], "codenum": i["BR_per_code"]})
    mock_data = [
        # {"id": "1", "Changjia": "123", "MaterialPN": "", "Description": "", "Power": "", "Number": "", "Location": "",
        #  "Customer": "", "Project_Code": "", "Phase": "", "OAP": "", "Device_Status": "", "BR_Status": "已借用",
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCode.objects.filter(BR_Status='歸還確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
            ...
            if request.POST.get('isGetData') == 'SEARCH':
                ...
                if request.POST.get('Borrower'):
                    checkAdaPow['BR_per'] = request.POST.get('Borrower')
                    if request.POST.get('BorrowerNum', None):
                        checkAdaPow['BR_per'] = request.POST.get('BorrowerNum')
                # Changjiasearch = request.POST.get('Changjia')
                # if Changjiasearch and Changjiasearch != "All":
                #     checkAdaPow['Changjia'] = Changjiasearch
                # Customerserach = request.POST.get('Customer')
        ...
        if request.POST:
            ...
            if request.POST.get('action') == "submit":
                ...
                createdic = {
                     ...
                     'Power': Power, 'OAP': OAP, 'Changjia': Changjia, 'Number': Number, 'Location': Location
                             }
                if AdapterPowerCode.objects.filter(Number=Number):
                    errMsgNumber = """编号已存在"""
                else:
                    # print(createdic)
                    AdapterPowerCode.objects.create(**createdic)
                    # print(Photo)
                    if Photo:
                        for f in Photo:  # 数据上允许传多张照片，实际生产环境中头像只会传一张
                            ...
                            empt.pic = f
                            empt.save()
                            AdapterPowerCode.objects.filter(Number=Number).first().Photo.add(empt)
            ...
            if request.POST.get('action') == "submit1":
                ...
                try:
                    with transaction.atomic():
                        AdapterPowerCode.objects.filter(id=id).update(**updatedic)
                        alert = 0
                except:
                    alert = '此数据%s正被其他使用者编辑中...' % id
                # print(Photo)
                if Photo:
                    for m in AdapterPowerCode.objects.filter(id=id).first().Photo.all():  # 每次接受图片前清除原来的图片，而不是叠加
                        # print(m.id)
                        PICS.objects.filter(
                            id=m.id).delete()
                    for f in Photo:  # 数据上允许传多张照片，实际生产环境中头像只会传一张
                        ...
                        empt.pic = f
                        empt.save()
                        AdapterPowerCode.objects.filter(id=id).first().Photo.add(empt)
            ...
            try:
                ...
                if 'MUTICANCEL' in str(request.body):
                    ...
                    for i in responseData['params']:
                        # print(i)
                        AdapterPowerCode.objects.get(id=i).delete()
                ...
                if 'ExcelData' in str(request.body):
                    ...
                    if startupload:
                        ...
                        for i in xlsxlist:
                            ...
                            if AdapterPowerCode.objects.filter(**Check_dic):#已经存在覆盖
                                AdapterPowerCode.objects.filter(
                                    **Check_dic).update(**modeldata)
                            else:#新增
                                AdapterPowerCode.objects.create(**modeldata)
...
def M_Keep(request):
    ...
    for i in AdapterPowerCode.objects.filter(BR_Status='續借確認').values('BR_per', 'BR_per_code').distinct().order_by('BR_per_code'):
        selectItem.append({"value": i['BR_per'],'codenum': i['BR_per_code']})
    mock_data = [
        # {"id": "1", "Changjia": "123", "MaterialPN": "", "Description": "", "Power": "", "Number": "", "Location": "",
        #  "Customer": "", "Project_Code": "", "Phase": "", "OAP": "", "Device_Status": "", "BR_Status": "已借用",
    ...
    if request.method == "POST":
        if request.POST:
            if request.POST.get('isGetData') == 'first':
                # mock_data
                mock_datalist = AdapterPowerCode.objects.filter(BR_Status='續借確認')
                for i in mock_datalist:
                    Photolist = []
                    for h in i.Photo.all():
                        Photolist.append(
